defmodule Pinchflat.Sources do
    ...
    [
      ...
      not directory_writable? &&
        {:storage_directory_unwritable,
         "Tubeless can't write to #{storage_directory(source)}, so no download can be saved. Check the volume's permissions."}
    ]
  ...
  def update_source(%Source{} = source, attrs, opts \\ []) do
    # Only internal callers (metadata storage, reconcile) may write the
    # app-managed filepath fields; HTTP updates must not (see Source.changeset).
    cast_opts = Keyword.take(opts, [:cast_internal_fields])

    case change_source(source, attrs, :initial, cast_opts) do
      %Ecto.Changeset{valid?: true} ->
        source
        |> maybe_change_source_from_url(attrs, cast_opts)
        |> maybe_change_indexing_frequency()
        |> maybe_assign_slug()
        |> commit_and_handle_tasks(opts)
  ...
  """
  def change_source(%Source{} = source, attrs \\ %{}, validation_stage \\ :pre_insert, opts \\ []) do
    Source.changeset(source, attrs, validation_stage, opts)
  end
  ...
  defp maybe_change_source_from_url(%Source{} = source, attrs, cast_opts \\ []) do
    case change_source(source, attrs, :pre_insert, cast_opts) do
