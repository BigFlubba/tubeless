  ...
  def render(assigns) do
    ...
    <div>
      ...
      <.modal id={@modal_id}>
        ...
        <p class="text-xs text-bodydark mb-6">
        ...
        <%= case @status do %>
          <% :idle -> %>
          <% :running -> %>
            <div class="rounded-lg border border-strokedark bg-meta-4 p-4 flex items-center gap-2">
              <.icon name="hero-arrow-path" class="h-4 w-4 animate-spin text-white" />
          ...
          <% :findings -> %>
            <div class="rounded-lg border border-red-500 bg-red-500/10 p-4">
              ...
              <ul class="max-h-64 overflow-y-auto rounded border border-strokedark/50 bg-black/20 p-3 space-y-1">
                <li :for={finding <- @findings} class="text-xs text-white font-mono break-words">
                  {finding}
                </li>
              </ul>
              <p class="text-xs text-bodydark mt-3">
                Nothing has been changed. Repairs depend on what's damaged and are safest done with
                Tubeless stopped, so they aren't automated here — back up your database file before
                attempting one. See
                <.link
                  href="https://github.com/CommunityMaintained/tubeless/wiki/Database-Repair"
                  target="_blank"
          ...
          <% :error -> %>
            <div class="rounded-lg border border-red-500 bg-red-500/10 p-4">
              <p class="text-sm text-red-400 font-semibold">The check could not run.</p>
              <p class="text-xs text-white font-mono mt-2 break-words">{@error}</p>
            </div>
        <% end %>
      </.modal>
    </div>
