defmodule PinchflatWeb.Sources.SourceHTML do
  ...
  @doc """
  Renders the Podcast tab. Mounted for every source — the dynamic feed works
  regardless of the podcast toggle; `static_podcast_state/1` decides what the
  static-delivery row says.
  """
  attr :source, :any, required: true
  attr :conn, :any, required: true

  def podcast_tab(assigns)

  @doc """
  Renders the Activity tab: the pinned unresolved failure (if any) and the
  timeline of recent and in-flight work for this source.

  `entries` and `unresolved` come from two independent queries
  (`Tasks.list_recent_activity_for_source/2` and
  `list_unresolved_activity_for_source/2`), so the pinned failure survives the
  timeline's cap.
  """
  attr :entries, :list, required: true
  attr :unresolved, :list, required: true

  def activity_tab(assigns)
  ...
  def blocking_banner(assigns) do
    ...
    >
      ...
      <.link
        ...
        class="shrink-0 rounded-md border border-current px-3 py-1.5 text-sm font-medium hover:opacity-80"
      >
        {action.label}
      </.link>
    </div>
  ...
  @doc """
  Everything the Podcast tab needs to know about the static (serve-in-place)
  delivery mode, resolved **once** per render:

    - `:publishing` — the source's profile has `Publish as Podcast` on
    - `:missing_url_base` — publishing, but the `podcast_url_base` setting is empty
    - `:pending` — publishing and configured, but `feed.xml` hasn't been written yet
      (export is debounced ~30s and can fail)
    - `:url` — the feed URL on the external static server, non-nil only once the
      feed actually exists on disk, so a URL that would 404 is never advertised
    - `:local_path` — the directory the static web server must serve

  Gathered in one place because each answer costs a `Settings.get!` (a DB read)
  and a stat of the feed on disk, and the template asks for several of them in
  more than one spot.
  """
  def static_podcast_state(source) do
    url_base = Settings.get!(:podcast_url_base)
    publishing = Pinchflat.Podcasts.PodcastExport.enabled?(source)
    generated = publishing && source.slug && Pinchflat.Podcasts.PodcastExport.feed_generated?(source)

    %{
      publishing: publishing,
      missing_url_base: publishing && is_nil(url_base),
      pending: !!(publishing && url_base && source.slug && !generated),
      url: if(generated && url_base, do: Pinchflat.Podcasts.StaticFeedLinks.self_url(url_base, source)),
      local_path: if(source.slug, do: Path.join(Pinchflat.Podcasts.PodcastExport.podcast_directory(), source.slug))
    }
  end
